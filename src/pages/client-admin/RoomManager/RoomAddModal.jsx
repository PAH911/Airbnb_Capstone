...
import { addRoom } from "./adminRoomSlice";
...
export default function RoomAddModal({ open, onClose, onSuccess, onFailed }) {
  const dispatch = useDispatch();
  const { addLoading } = useSelector((state) => state.adminRoom);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await dispatch(addRoom({ room: formData, imageFile }));
      if (addRoom.fulfilled.match(result)) {
        onSuccess && onSuccess();
        handleClose();
      } else {
        console.error("=== Add Room Failed ===", result.payload);
        // Kiểm tra nếu lỗi liên quan đến upload ảnh
        if (
          result.payload &&
          (result.payload.includes("upload") ||
            result.payload.includes("image") ||
            result.payload.includes("hình"))
        ) {
          onFailed && onFailed(result.payload);
        } else {
          onFailed && onFailed(result.payload || "Thêm phòng thất bại");
        }
      }
    } catch (error) {
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-4xl transform overflow-hidden rounded-3xl bg-gradient-to-br from-gray-800 to-gray-900 p-8 text-left align-middle shadow-2xl transition-all border border-gray-700">
                ...
                >
                  <HomeOutlined className="text-yellow-400" /> Thêm phòng mới
                </Dialog.Title>
                  ...
                  <div className="flex justify-end space-x-4 pt-6 border-t border-gray-700">
                    ...
                    >
                      {addLoading ? (
                        ...
                        <>
                          <CheckCircleOutlined /> Thêm phòng
                        </>
                      )}
                    </button>
                  </div>
